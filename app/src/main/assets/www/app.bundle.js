...
// ====== STORAGE ======
  ...
  function applyDeptDetailsEnabled(enabled){
    // This toggle only controls the issue classification fields (category/severity/action + issue images).
    // Summary + General Notes remain always available and are NOT tied to the checkbox.
    ...
    const ids = [
      "deptCategory","deptCategoryOther","deptSeverity","deptIssueDesc","deptAction","btnAttachIssue","btnCameraIssue"
    ];
    if(block){
      block.style.display = enabled ? "" : "none";
    }
    if(enabled){
      setTimeout(()=>{ try{ $("deptCategory")?.focus(); }catch(e){} }, 50);
    }
    ids.forEach(id=>{
      const el = $(id);
      if(!el) return;
...
Source: dnata dCROM chapter 2.7`
  ...
  function wireEvents() {
    ...
    $("btnFinalCopy").addEventListener("click", copyFinalSummary);
    ...
    $("btnNewDeptIssueNote")?.addEventListener("click", clearDeptIssueDraftUI);
...
function buildFinalReportHTML() {
  ...
  // pick a global shift if available (prefer OPERATIONS)
  let globalShift = "";
  try{
    const op = loadReport("OPERATIONS");
  ...
  // Printable header (repeats via CSS fixed positioning)
  parts.push(`
    <div class="finalPrintHeader">
      <div style="font-weight:900; font-size:14px;">DAILY DUTY REPORT</div>
      <div>dnata Catering & Retail – Jordan</div>
      <div class="finalMetaRow">
        <div class="item"><span class="finalLabelInline">Date:</span> ${escapeHtml(new Date().toLocaleDateString())}</div>
        <div class="item"><span class="finalLabelInline">Shift:</span> ${escapeHtml(globalShift || "-")}</div>
        <div class="item"><span class="finalLabelInline">Generated:</span> ${escapeHtml(createdAt)}</div>
      </div>
    </div>
  `);

  parts.push(`<div class="finalReport">`);

  // Aggregations for summary
  const sevCount = {LOW:0, MEDIUM:0, HIGH:0, CRITICAL:0};
  const catCount = {};
  let sectionsWithIssues = 0;
  let totalNotes = 0; // counts section-issue as 1, plus each safety note
  ...
  // Fixed order, always show all sections
  DEPARTMENTS.forEach(d => {
    const r = loadReport(d.key);

    const lines = [
      ...nonEmptyLines(r?.summary||""),
      ...nonEmptyLines(r?.notes||"")
    ];
    ...
    // Determine if section has any issues/data
    const hasIssue = (lines.length > 0) || (imgs.length > 0) || (sectionAction.length > 0) || (d.key === "SAFETY" && safetyNotes.length > 0);
    ...
    if(!hasIssue){
      parts.push(`<div class="finalGood"><span class="sevDot low"></span> General status is good</div>`);
    } else {
      sectionsWithIssues += 1;

      // For non-safety sections: support multiple saved issue notes
      if(d.key !== "SAFETY"){
        ...
        if(issueNotes.length){
          issueNotes.forEach((n, i)=>{
            ...
            parts.push(`
              <div style="margin-top:${i===0?0:8}px;">
                <div style="font-weight:900;">${i+1}. Note</div>
                <div><span class="finalLabelInline">Category:</span> ${escapeHtml(cat)}</div>
                <div><span class="finalLabelInline">Severity:</span> <span class="sevDot ${dotClass(sev)}"></span> ${escapeHtml(sev)}</div>
              </div>
            `);
            if(descLines.length){
              parts.push(`<ul class="finalBullets">${descLines.map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul>`);
            }
            parts.push(renderImgs(nImgs));
            if(act){
              parts.push(`<div class="finalAction"><span class="finalLabelInline">Action Taken:</span> ${escapeHtml(act)}</div>`);
            }
          });
        } else {
          totalNotes += 1;
          ...
          parts.push(`
            <div><span class="finalLabelInline">Category:</span> ${escapeHtml(sectionCategory)}</div>
            <div><span class="finalLabelInline">Severity:</span> <span class="sevDot ${dotClass(sectionSeverity)}"></span> ${escapeHtml(sectionSeverity)}</div>
          `);

          if(lines.length){
            parts.push(`<ul class="finalBullets">${lines.map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul>`);
          }

          parts.push(renderImgs(imgs));

          if(sectionAction){
            parts.push(`<div class="finalAction"><span class="finalLabelInline">Action Taken:</span> ${escapeHtml(sectionAction)}</div>`);
          }
        }
      } else {
        // SAFETY section: each safety note is a numbered item
        if(safetyNotes.length === 0){
          // still has issue because maybe images/action/lines were added (rare); show as generic
          totalNotes += 1;
          incCat(sectionCategory);
          incSev(sectionSeverity);
          ...
          parts.push(`<div class="muted" style="display:none"></div>`); // (keeps layout stable; hidden in print)
          safetyNotes.forEach((n, i)=>{
            ...
            const noteLines = nonEmptyLines(noteText);
            const nImgs = Array.isArray(n.images) ? n.images : [];
            const act = (n.action_taken || "").trim();

            parts.push(`
              <div style="margin-top:${i===0?0:8}px;">
                <div style="font-weight:900;">${i+1}. Safety Note</div>
                <div><span class="finalLabelInline">Category:</span> ${escapeHtml(cat)}</div>
                <div><span class="finalLabelInline">Severity:</span> <span class="sevDot ${dotClass(sev)}"></span> ${escapeHtml(sev)}</div>
              </div>
            `);
            ...
            if(noteLines.length){
              parts.push(`<ul class="finalBullets">${noteLines.map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul>`);
            }
  ...
  parts.push(`
    <div class="finalSummaryBox">
      <div class="finalSummaryTitle">REPORT SUMMARY</div>

      <div style="margin-bottom:8px;">
        <div><span class="finalLabelInline">Total Sections:</span> ${DEPARTMENTS.length}</div>
        <div><span class="finalLabelInline">Sections with Issues:</span> ${sectionsWithIssues}</div>
        <div><span class="finalLabelInline">Total Notes:</span> ${totalNotes}</div>
      </div>

      <div class="finalTwoCol">
        <div>
          <div style="font-weight:900; margin-bottom:4px;">Severity Distribution</div>
          <div><span class="sevDot critical"></span> CRITICAL: ${sevCount.CRITICAL||0}</div>
          <div><span class="sevDot high"></span> HIGH: ${sevCount.HIGH||0}</div>
          <div><span class="sevDot medium"></span> MEDIUM: ${sevCount.MEDIUM||0}</div>
          <div><span class="sevDot low"></span> LOW: ${sevCount.LOW||0}</div>
        </div>

        <div>
          <div style="font-weight:900; margin-bottom:4px;">Category Distribution</div>
          ${sortedCats.length ? sortedCats.map(([k,v])=>`<div>${escapeHtml(k)}: ${v}</div>`).join("") : `<div class="muted">No categories.</div>`}
        </div>
      </div>

      <div style="margin-top:10px; color: rgba(0,0,0,0.70);">
  ...
  // Printable footer
  parts.push(`
    <div class="finalPrintFooter">
      Duty Report Web — Enable "Headers & Footers" in print dialog to include Page X of Y.
    </div>
  `);

  return { html: parts.join(""), createdAt };
}
